...
export function useImageFiles() 
{
      ...
      function buildChanges(): ImageChanges 
      {
            ...
            const removedImageIds = initialImagesRef.current.filter((original) =>
                  !existingImages.some((image) => 
                        image.imageId === original.imageId)).map((image) => 
                              image.imageId);

            const updatedImages = existingImages.flatMap((image) => 
            {
                  const original = initialImagesRef.current.find((item) =>
