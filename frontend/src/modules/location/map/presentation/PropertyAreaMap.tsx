...
export function PropertyAreaMap({
  ...
  const projectionListenerRef = useRef<google.maps.MapsEventListener | null>(null);
  ...
  }, [mode, onPolygonChange, onSetMarker, polygon]);
    ...
    async function setupMap(): Promise<void> {
      ...
      try {
        ...
        if (!readOnly) {
          ...
          projectionListenerRef.current = map.addListener("projection_changed", () => {
            ...
            draw.start();
            draw.on("ready", () => {
              const initialFeature = toPolygonFeature(polygonRef.current);
  ...
  useEffect(() => {
    ...
    if (!draw || readOnly || !isReady) {
      return;
    }

    const feature = toPolygonFeature(polygon);
    const currentPolygon = extractPolygon(draw.getSnapshot());

    if (JSON.stringify(currentPolygon) === JSON.stringify(polygon)) {
      return;
    }
